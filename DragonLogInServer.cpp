...
int CheckIDAutherizing(t_packet &packet, const int cn)
{
	...
	case COnePass::BT_BANNED: // -3 : 돈내!...		//2001/01/28 ZHH
	{
		::MyLog(0, "User ! ID:('%s') PW:('%s') Tried to login Banned account", pCN->id, pCN->pw);

		packet.h.header.type = CMD_BANNED_ACCOUNT;
		packet.h.header.size = 0;
		::QueuePacket(connections, cn, &packet, 1);

		::closeconnection(connections, cn, CCT_WRONG_BANNED_ACCOUNT);
	}break;
